...
Of course, we can repeat this for all sponge networks. First, we make a list of all names from the folders containing the networks. Then we find the matching BIOM files. Finally, we store these networks as graphml files instead, so they can be imported in other software. We also store them in a named list so they can be used for other analyses. 

```{r taxdict_loop, eval=FALSE}
# Get a list of all files and remove the .gml extension
network_names <- list.files("sponges_networks")
network_list <- list()
for (name in network_names){
  network = read_graph(paste(network_loc, name, sep=''), format='gml')
  # read BIOM file
  biom_file <- import_biom(paste(biom_loc, substr(name, 1, nchar(name)-4), '.biom', sep=''))
  tax <- data.frame(tax_table(biom_file))
  for (col in colnames(tax)){
    network <- set.vertex.attribute(network, value=tax[[col]], name=col)
    network_list[[substr(name, 1, nchar(name)-4)]] <- network
    write_graph(network, format="graphml")
  }
}
